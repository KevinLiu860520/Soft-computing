...
namespace R08546002YCLiuAss06
{
    ...
    class SugenoFuzzySystem : IFuzzyInferencing
    {
        ...
        public double CrispInCrispOutInferencing(double[] conditions)
        {
            double OutputSum = 0; double FirestrengthSum = 0.0;
            foreach (SugenoIfThenRule ifr in rules)
            {   
                double Output =  ifr.CrispInCrispyOutInferencing(conditions);
                FirestrengthSum += ifr.FiringStrength;
                OutputSum += Output * ifr.FiringStrength;
            }
            if (ISAverage) return OutputSum / FirestrengthSum;
            else return OutputSum;
        }
    }
    class TsukamotoFuzzySystem : IFuzzyInferencing
    {
        TsukamotoIfThenRule[] rules;
        public bool IsWeightedAverage { get; set; } = true;

        public void ConstructSystem(DataGridView dgv)
        {
            rules = new TsukamotoIfThenRule[dgv.Rows.Count];
            FuzzySet output;
            for (int r = 0; r < dgv.Rows.Count; r++)
            {
                FuzzySet[] inputs = new FuzzySet[dgv.Columns.Count - 1];
                ...
                output = (FuzzySet)dgv.Rows[r].Cells[dgv.Columns.Count - 1].Value;
                rules[r] = new TsukamotoIfThenRule(inputs, output);
            }
        }
        ...
        {
            double OutputSum = 0; double FirestrengthSum = 0.0;
            foreach (TsukamotoIfThenRule ifr in rules)
            {
                OutputSum += ifr.CrispInCrispOutInferencing(conditions);
                FirestrengthSum += ifr.FiringStrength;
            }
            if (IsWeightedAverage) return OutputSum / FirestrengthSum;
            else return OutputSum;
        }
    }
}

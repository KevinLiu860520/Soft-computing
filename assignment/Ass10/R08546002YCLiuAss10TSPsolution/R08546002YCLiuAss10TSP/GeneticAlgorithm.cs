...
{    
    ...
    public class GeneticAlgorithm<T>
    {
        ...
        [Category("GA setting"), Description("Least fitness fraction should be between 0 and 0.5")]
        ...
        private void performSelectionOperation()
        {
            ...
            switch (SelectionMode)
            {
                ...
                case SelectionType.Hybrid:
                    double sumFitness = 0;
                    double[] sumProb = new double[limit];
                    for (int i = 0; i < limit; i++)
                    {
                        sumFitness += fitnessValues[i];
                    }
                    sumProb[0] = fitnessValues[0] / sumFitness;
                    for (int i = 1; i < limit; i++)
                    {
                        sumProb[i] = sumProb[i-1] + fitnessValues[i] / sumFitness;
                    }
                    for (int i = 0; i < populationSize; i++)
                    {
                        double rndDouble = randomizer.NextDouble(); // first chromosome
                        double rndDoubleCompare = randomizer.NextDouble(); // second chromosome
                        for (int j = 0; j < limit; j++)
                        {
                            if (Math.Max(rndDouble, rndDoubleCompare) >= sumProb[j]) // select large
                            {
                                indices[i] = j;
                            }
                        }
        ...
        {
            do RunOneIterarion(); while (iterationCount < iterationLimit);
            return true;
        }

...
namespace GeneticAlgorithmLibrary
{
    public class RealNumberGASolver : GeneticAlgorithm<double>
    {
        ...
        [Category("RealNumber GA setting"), Description("Default point cut.")]
        ...
        [Category("RealNumber Setting"), Description("Degree of uniformity")]
        public double MutationFactor
        {
            get => mutationFactor;
            set
            {
                if (value <= 1 && value >= 0)mutationFactor = value;
            }
        ...
        public override void MutateAChild(int parentIdx, int childIdx)
        {
            ...
            if (Randomterm > 0.5)
            {
                ...
                chromosomes[childIdx][position] = chromosomes[parentIdx][position] +
                    alpha * mutationFactor * (upperBounds[position] - chromosomes[parentIdx][position]);
            }
            else
            {
                for (int i = 0; i < numberOfGenes; i++)
                ...
                chromosomes[childIdx][position] = chromosomes[parentIdx][position] -
                    alpha * mutationFactor * (chromosomes[parentIdx][position] - lowerBounds[position]);
            }
        }
    }
}
